...
bool ModuleAudio::Init()
{
	...
	if((init & flags) != flags)
	{
		...
		App->Console.AddLog("Could not initialize Mixer lib. Mix_Init: %s", Mix_GetError());
		ret = true;
	}
	...
	if(Mix_OpenAudio(44100, MIX_DEFAULT_FORMAT, 2, 2048) < 0)
	{
		...
		App->Console.AddLog("SDL_mixer could not initialize! SDL_mixer Error: %s\n", Mix_GetError());
		ret = true;
	}
...
bool ModuleAudio::PlayFx(unsigned int id, int repeat)
{
	...
	std::list<Mix_Chunk*>::iterator ot = fx.begin();
	std::advance(ot, id - 1);
	if((*ot) != NULL)
	{
		Mix_PlayChannel(-1, (*ot), repeat);
		ret = true;
	}
